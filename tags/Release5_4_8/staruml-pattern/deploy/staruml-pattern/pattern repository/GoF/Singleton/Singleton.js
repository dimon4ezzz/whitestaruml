...
// Class (Singleton)
if (!HasSameOperation(singleton, "Instance", "", vkPublic, new Array())){
  var singleOp = factory.CreateOperation(singleton);
  singleOp.Name = "Instance";
  var param = factory.CreateParameter(singleOp);
  param.Type_ = singleton;
  param.DirectionKind = 3;
}


var assoc = GetSingleAssication(owner, "" , singleton, "" , akNone, "", false, singleton, "instance", akNone, "", true);
var assocEnd1 = assoc.GetConnectionAt(0);
var assocEnd2 = assoc.GetConnectionAt(1);
assocEnd1.IsNavigable = false;
assocEnd2.Name = "instance";
assocEnd2.TargetScope = 1;
assocEnd2.Visibility = 2;
...
var singleView = GetSingleClassifierView(dgm.DiagramView, singleton);
if(!ViewExist){
  singleView.Left = 100;
  singleView.Top = 100;
}

// Association View (Singleton)
var assocView = GetSingleAssociationView(dgm.DiagramView, assoc, singleView, singleView);
if(!ViewExist){
