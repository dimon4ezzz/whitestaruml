...
// Global Val
var ConPrototypeGenList = new Array();
...
var prototypeOp = GetSingleOperation(prototype, "Clone", "", vkPublic, new Array());
prototypeOp.Name = "Clone";

// Class (ConcretePrototype)
// Class (Client)

// Association (Client-Prototype)
var assoc = GetSingleAssication(owner, "" ,client , "" , akNone, "", false, prototype, "prototype", akNone, "", true);
var assocEnd1 = assoc.GetConnectionAt(0);
var assocEnd2 = assoc.GetConnectionAt(1);
assocEnd1.IsNavigable = false;
assocEnd2.Name = "prototype";

// Generalization (Prototype-ConcretePrototype)
for (var i = 0; i < concretePrototype.length; i++){
    var gen = GetSingleGeneralization(owner, "", prototype, concretePrototype[i]);
    ConPrototypeGenList[ConPrototypeGenList.length] = gen;
}
...
// Class View (Prototype)
var prototypeView = GetSingleClassifierView(dgm.DiagramView, prototype);
if(!ViewExist){
  prototypeView.Left = 354;
  prototypeView.Top = 100;
}

// Class View (ConcretePrototype)
var conProto_len = concretePrototype.length - 1;
var conPrototypeView = GetSingleClassifierView(dgm.DiagramView, concretePrototype[conProto_len]);
if(!ViewExist){
  conPrototypeView.Left = 335;
  conPrototypeView.Top = 304;
}

// Association View (Client-Prototype)
var assocView = GetSingleAssociationView(dgm.DiagramView, assoc, clientView, prototypeView);
if(!ViewExist){
  assocView.LineStyle = 0;
}

// Generalization View (Prototype-ConcretePrototype)
var conProGen_len = concretePrototype.length - 1;
var gen = ConPrototypeGenList[conProGen_len];
var genView = GetSingleGeneralizationView(dgm.DiagramView, gen, prototypeView, conPrototypeView);
