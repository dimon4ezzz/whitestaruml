...
// Global Val
var ProRealSubAssoList = new Array();
var SubProGenList = new Array();
...
// Class (Subject)
subject.IsAbstract = true;

var subjectOp = GetSingleOperation(subject, "Request", "", vkPublic, new Array());
subjectOp.Name = "Request";

// Class (RealSubject)
// Class (Proxy)

// Generalization (Subject-RealSubject)
var realSubjectGen = GetSingleGeneralization(owner, "", subject, realSubject);

// Generalization (Subject-Proxy)
for (var i = 0; i < proxy.length; i++){
    var gen = GetSingleGeneralization(owner, "", subject, proxy[i]);
    SubProGenList[SubProGenList.length] = gen;
}

// Association (Proxy-RealSubject)
for (var i = 0; i < proxy.length; i++){
    var assoc = GetSingleAssication(owner, "" ,proxy[i] , "" , akNone, "", false, realSubject, "RealSubject", akNone, "", true);
    var assocEnd1 = assoc.GetConnectionAt(0);
    var assocEnd2 = assoc.GetConnectionAt(1);
    assocEnd1.IsNavigable = false;
    assocEnd2.Name = "RealSubject";
    ProRealSubAssoList[ProRealSubAssoList.length] = assoc;
}
...
// Class View (Subject)
var subjectView = GetSingleClassifierView(dgm.DiagramView, subject);
if(!ViewExist){
  subjectView.Left = 198;
  subjectView.Top = 88;
}

// Class View (RealSubject)
var realSubjectView = GetSingleClassifierView(dgm.DiagramView, realSubject);
if(!ViewExist){
  realSubjectView.Left = 96;
  realSubjectView.Top = 216;
}

// Class View (Proxy)
var pro_len = proxy.length - 1;
var proxyView = GetSingleClassifierView(dgm.DiagramView, proxy[pro_len]);
if(!ViewExist){
  proxyView.Left = 308;
  proxyView.Top = 216;
}

// Generalization View (Subject-RealSubject)
var realSubjectGenView = GetSingleGeneralizationView(dgm.DiagramView, realSubjectGen, subjectView, realSubjectView);
if(!ViewExist){
  realSubjectGenView.LineStyle = 0;
  realSubjectGenView.Points.BeginUpdate();
  realSubjectGenView.Points.ClearPoints();
  realSubjectGenView.Points.AddPoint(136, 216);
  realSubjectGenView.Points.AddPoint(136, 188);
  realSubjectGenView.Points.AddPoint(236, 188);
  realSubjectGenView.Points.AddPoint(236, 143);
  realSubjectGenView.Points.EndUpdate();
}

// Generalization View (Subject-Proxy)

var proSubGen_len = SubProGenList.length - 1;
var gen = SubProGenList[proSubGen_len];
var genView = GetSingleGeneralizationView(dgm.DiagramView, gen, subjectView, proxyView);
if(!ViewExist){
  genView.LineStyle = 0;
  genView.LineStyle = 0;
  genView.Points.BeginUpdate();
  genView.Points.ClearPoints();
  genView.Points.AddPoint(347, 216);
  genView.Points.AddPoint(347, 188);
  genView.Points.AddPoint(236, 188);
  genView.Points.AddPoint(236, 143);
  genView.Points.EndUpdate();
}

// Association View (Proxy-RealSubject)
var proRealSub_len = ProRealSubAssoList.length - 1;
var assoc = ProRealSubAssoList[proRealSub_len];
var AssoView = GetSingleAssociationView(dgm.DiagramView, assoc, proxyView, realSubjectView);
