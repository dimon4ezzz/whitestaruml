...
var
  P: IProfile;
begin
  if FStarUMLApp.ExtensionManager.FindIncludedProfile(PROFILE_JAVA) = nil then begin
    P := FStarUMLApp.ExtensionManager.FindAvailableProfile(PROFIlE_JAVA);
    if P <> nil then begin
      if Application.MessageBox(C_QUERY_REVERSE_PROFILE_LOAD, PChar(Application.Title),
        MB_YESNO + MB_ICONQUESTION) = IDYES then begin
        FStarUMLApp.ExtensionManager.IncludeProfile(PROFILE_JAVA);
        Result := True;
      end
      else
        Result := False;
