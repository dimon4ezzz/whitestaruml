...
// Class (Context)
var contextOp1 = GetSingleOperation(context, "Request", "", vkPublic, new Array());
contextOp1.Name = "Request";

// Class (State)
state.IsAbstract = true;
var stateOp1 = GetSingleOperation(state, "Handle", "", vkPublic, new Array());
stateOp1.Name = "Handle";

// Class (ConcreteState)

// Association (Context-State)
var assoc = GetSingleAssication(owner, "" , context, "" , akAggregate, "", false, state, "state", akNone, "", true);
var assocEnd1 = assoc.GetConnectionAt(0);
var assocEnd2 = assoc.GetConnectionAt(1);
assocEnd1.IsNavigable = false;
assocEnd2.Name = "state";
assocEnd1.Aggregation = 1;

// Generalization (State-ConcreteState)
for (var i = 0; i < concreteState.length; i++){
    var gen = GetSingleGeneralization(owner, "", state, concreteState[i]);
    ConStateGenList[ConStateGenList.length] = gen;
}
...
// Class View (State)
var stateView = GetSingleClassifierView(dgm.DiagramView, state);
if(!ViewExist){
  stateView.Left = 350;
  stateView.Top = 100;
}

// Class View (ConcreteState)
var conState_len = concreteState.length - 1;
var conStateView = GetSingleClassifierView(dgm.DiagramView, concreteState[conState_len]);
if(!ViewExist){
  conStateView.Left = 100;
  conStateView.Top = 300;
}

// Association View (Context-State)
var assocView = GetSingleAssociationView(dgm.DiagramView, assoc, contextView, stateView);
if(!ViewExist){
  assocView.LineStyle = 0;
}

// Generalization View (Visitor-ConcreteVisitor)
var conVisGen_len = concreteState.length - 1;
var gen = ConStateGenList[conVisGen_len];
var genView = GetSingleGeneralizationView(dgm.DiagramView, gen, stateView, conStateView);
