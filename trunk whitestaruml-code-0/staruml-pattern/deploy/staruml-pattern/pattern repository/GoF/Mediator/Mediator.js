...
// Class (Mediator)
mediator.IsAbstract = true;

// Class (ConcreteMediator)
// Class (Colleague)
colleague.IsAbstract = true;

// Generalization (Mediator-ConcreteMediator)
var gen = GetSingleGeneralization(owner, "", mediator, concreteMediator);

// Association (Mediator-Colleague)
var assoc1 = GetSingleAssication(owner, "" ,colleague , "" , akNone, "", false, mediator, "mediator", akNone, "", true);
var assoc1End1 = assoc1.GetConnectionAt(0);
var assoc1End2 = assoc1.GetConnectionAt(1);
assoc1End1.IsNavigable = false;
assoc1End2.Name = "mediator";
...
// Class View (Mediator)
var mediView = GetSingleClassifierView(dgm.DiagramView, mediator);
if(!ViewExist){
  mediView.Left = 100;
  mediView.Top = 100;
}

// Class View (ConcreteMediator)
var conMediView = GetSingleClassifierView(dgm.DiagramView, concreteMediator);
if(!ViewExist){
  conMediView.Left = 84;
  conMediView.Top = 250;
}

// Class View (Colleague)
var collView = GetSingleClassifierView(dgm.DiagramView, colleague);
if(!ViewExist){
  collView.Left = 373;
  collView.Top = 100;
}

// Association View (Mediator-Colleague)
var AssoView = GetSingleAssociationView(dgm.DiagramView, assoc1, collView, mediView);
if(!ViewExist){
  AssoView.LineStyle = 0;
}

// Generalization View (Mediator-ConcreteMediator)
var genView = GetSingleGeneralizationView(dgm.DiagramView, gen, mediView, conMediView);
if(!ViewExist){
  genView.LineStyle = 0;
}
