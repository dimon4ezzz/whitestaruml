...
// Class (Originator)
if (!HasSameOperation(originator, "SetMemento", "", vkPublic, new Array())){
  var OriginOp1 = factory.CreateOperation(originator);
  OriginOp1.Name = "SetMemento";
  var subOp1Param1 = factory.CreateParameter(OriginOp1);
  subOp1Param1.Name = "m";
  subOp1Param1.Type_ = memento;
}

var OriginOp2 = GetSingleOperation(originator, "CreateMemento", "", vkPublic, new Array());
OriginOp2.Name = "CreateMemento";

// Class (Memento)
var MemOp1 = GetSingleOperation(memento, "GetState", "", vkPublic, new Array());
MemOp1.Name = "GetState";
var MemOp2 = GetSingleOperation(memento, "SetState", "", vkPublic, new Array());
MemOp2.Name = "SetState";

// Class (Caretaker)

// Association (Caretaker-Memento)
var assoc = GetSingleAssication(owner, "" ,caretaker , "" , akAggregate, "", false, memento, "menento", akNone, "", true);
var assocEnd1 = assoc.GetConnectionAt(0);
var assocEnd2 = assoc.GetConnectionAt(1);
assocEnd2.Name = "menento";
assocEnd1.IsNavigable = false;
assocEnd1.Aggregation = akAggregate;

// Dependency (Originator-Memento)
var dePen = GetSingleDependency(owner, originator, memento, "", "");
// Model
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
// View
...
// Class View (Originator)
var originView = GetSingleClassifierView(dgm.DiagramView, originator);
if(!ViewExist){
  originView.Left = 100;
  originView.Top = 100;
}
...
// Class View (Memento)
var memenView = GetSingleClassifierView(dgm.DiagramView, memento);
if(!ViewExist){
  memenView.Left = 300;
  memenView.Top = 100;
}

// Class View (Caretaker)
var careView = GetSingleClassifierView(dgm.DiagramView, caretaker);
if(!ViewExist){
  careView.Left = 500;
  careView.Top = 100;
}

// Association View (Originator-Memento)
var assocView = GetSingleAssociationView(dgm.DiagramView, assoc, careView, memenView);
if(!ViewExist){
  assocView.LineStyle = 0;
}

// Dependency View (Originator-Memento)
var depenView = GetSingleDependencyView(dgm.DiagramView, dePen, originView, memenView);
if(!ViewExist){
