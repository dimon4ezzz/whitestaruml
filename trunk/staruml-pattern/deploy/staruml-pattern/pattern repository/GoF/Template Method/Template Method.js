...
// Generalization (AbstractClass-ConcreteClass)
for (var i = 0; i < concreteClass.length; i++){
    var gen = GetSingleGeneralization(owner, "", abstractClass, concreteClass[i]);
    ConGenList[ConGenList.length] = gen;
}
...
var abstractClassView = GetSingleClassifierView(dgm.DiagramView, abstractClass);
if(!ViewExist){
  abstractClassView.Left = 102;
  abstractClassView.Top = 100;
}

// Class View (ConcreteClass)
var conClass_len = concreteClass.length - 1;
var concreteClassView = GetSingleClassifierView(dgm.DiagramView, concreteClass[conClass_len]);
if(!ViewExist){
  concreteClassView.Left = 101;
  concreteClassView.Top = 228;
}

// Generalization View (AbstractClass-ConcreteClass)
var conGen_len = concreteClass.length - 1;
var gen = ConGenList[conGen_len];
var genView = GetSingleGeneralizationView(dgm.DiagramView, gen, abstractClassView, concreteClassView);
