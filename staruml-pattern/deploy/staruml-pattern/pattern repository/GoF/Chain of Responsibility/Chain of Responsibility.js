...
// Global Val
var ConCreHandleGenList = new Array();
...
// Class (Handler)
handler.IsAbstract = true;
var handlerOp1 = GetSingleOperation(handler, "HandleRequest", "", vkPublic, new Array());
handlerOp1.Name = "HandleRequest";

// Class (ConcreteHandler)

// Generalization (Handler-ConcreteHandler)
for (var i = 0; i < concreteHandler.length; i++){
    var gen = GetSingleGeneralization(owner, "", handler, concreteHandler[i]);
    ConCreHandleGenList[ConCreHandleGenList.length] = gen;
}

// Association (Client-Handler)
var assoc1 = GetSingleAssication(owner, "" ,client, "" , akNone, "", false, handler, "", akNone, "", true);
...
var assoc2 = GetSingleAssication(owner, "" ,handler, "" , akNone, "", false, handler, "successor", akNone, "", true);
var assoc2End1 = assoc2.GetConnectionAt(0);
var assoc2End2 = assoc2.GetConnectionAt(1);
assoc2End2.Name = "successor";
...
// Class View (Handler)
var handlerView = GetSingleClassifierView(dgm.DiagramView, handler);
if(!ViewExist){
  handlerView.Left = 314;
  handlerView.Top = 100;
}

// Class View (ConcreteHandler)
var conHan_len = concreteHandler.length - 1;
var conCreHanView = GetSingleClassifierView(dgm.DiagramView, concreteHandler[conHan_len]);
if(!ViewExist){
  conCreHanView.Left = 312;
  conCreHanView.Top = 254;
}

// Association View (Client-Handler)
var assoc1View = GetSingleAssociationView(dgm.DiagramView, assoc1, clientView, handlerView);
if(!ViewExist){
  assoc1View.LineStyle = 0;
}

// Association View (Handler-Handler)
var assoc2View = GetSingleAssociationView(dgm.DiagramView, assoc2, handlerView, handlerView);
if(!ViewExist){
  assoc2View.LineStyle = 0;
}

// Generalization View (Handler-ConcreteHandler)
var conHanGen_len = concreteHandler.length - 1;
var gen = ConCreHandleGenList[conHanGen_len];
var genView = GetSingleGeneralizationView(dgm.DiagramView, gen, handlerView, conCreHanView);
if(!ViewExist){
  genView.LineStyle = 0;
}
